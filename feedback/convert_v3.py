#!/usr/bin/env python3
"""Convert raw text extracted from PDF v3 to markdown matching sua-1.md convention."""
import re
from pathlib import Path

RAW = Path(__file__).parent / "v3-raw-flow.txt"
OUT = Path(__file__).parent.parent / "thesis" / "khoa-luan-tot-nghiep-sua-2.md"
...
def classify_line(line: str) -> str:
    """Return kind: page, blank, h1, h2, h3, h4, bullet, subbullet, text."""
    s = line.strip()
    if not s:
        return 'blank'
...
def main():
    ...
    while i < n:
        ...
        def merge_heading_continuations(start_i, base_text):
            """Merge short text-kind continuation lines into the heading."""
            merged = base_text
            j = start_i + 1
            while j < n:
                nxt_kind = kinds[j]
        ...
        if kind == 'h1':
            ...
            out += ['---', '', f'# {merged}', '']
            i = next_i
            # Special case: when entering DANH MỤC TÀI LIỆU THAM KHẢO,
            # switch to reference-list mode for the rest of the document
            if 'DANH MỤC TÀI LIỆU THAM KHẢO' in merged:
                # Collect everything to end as reference list
                buf = []
                while i < n:
                    ss = lines[i].strip()
                    kk = kinds[i]
                    ...
                    buf.append(ss)
                    i += 1
                # Join, then split by item-start pattern
                full = ' '.join(buf)
                # Split into items: each item begins with "<digit>. " preceded by space or start
                import re as _re
                # Mark sections A./B.
                ...
                for part in full.split('\n'):
                    ...
                    if m_item:
                        ...
                        # Pull any URLs onto same line; replace any " https://" splits
        ...
        if kind in ('bullet', 'subbullet', 'bullet-marker', 'subbullet-marker'):
            ...
            if kind in ('bullet-marker', 'subbullet-marker'):
                # Content is on a following non-blank line
                text = ''
                j = i + 1
